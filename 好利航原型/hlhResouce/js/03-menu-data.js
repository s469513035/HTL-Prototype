...
/* 整柜财务按成本流转顺序排：
 * 成本侧 预估 → 代理账单(导入/对账/生成付款单) → 代理成本明细(分摊到委托单，散拼再到订单) → 付款单管理
 * 收入侧 应收费用明细 → 应收收款管理
 * 两侧走完 → 单票利润分析表（按委托单看收付与利润）
 * 注：原「单票成本明细」(fcl-shipment-cost) 的菜单入口已撤，其能力由利润分析表承接；
 *     TC 与二级分摊代码仍保留，未删表。 */
...
/* 收付两侧都走完之后的结账视角：一张委托单到底赚没赚。放在最后一项 */
{id:'fcl-profit',label:'单票利润分析表',page:'fcl',tab:'fcl-profit'}
]}
]},
