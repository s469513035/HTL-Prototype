...
/* 配柜状态整列去掉：配柜与否已经由配舱状态那条线表达，两列状态并存还要
 * statusMatch 同时匹配两列，插页点「已出仓」会把另一列同名的行也带出来。
 * 提单号改 Job No（与 Job/主单管理对齐），柜号后面补封签号（两者总是成对出现）。 */
addPrototypeTable('wh-final-alloc','配舱计划','配舱单号|标签编号|Job No|国家|柜号|封签号|配舱状态|票数|运输方式|件数|收货实重|收货方数|操作',['待出仓','已找货','可出仓','已出仓','海外已到仓'],[
    ['ZPCD-20260626-001','260626-1','FBK-20260626001','科特迪瓦','','','待出仓','3','空运','5','5','0.000005'],
    ['ZPCD-20260625-002','260625-2','FBK-20260625002','塞内加尔','','','可出仓','2','海运','4','4','0.000004'],
    ['ZPCD-20260625-001','260625-1','FBK-20260625001','尼日利亚','','','已找货','1','海运','1','1','0.000001'],
    ['20260624-终配001','260624-1','FBK-20260624001','加纳','GH-20260624-001','SL-20260624-001','已出仓','1','海运','2','2','0.000002'],
    ['ZPCD-20260622-001','260622-2','FBK-20260622001','塞内加尔','GH-20260622-001','SL-20260622-001','已出仓','2','海运','4','4','0.000004'],
    ['ZPCD-20260620-001','260620-1','FBK-20260620001','尼日利亚','','','待出仓','1','海运','3','3','0.000003'],
    ['ZPCD-20260615-001','260615-1','FBK-20260615001','多哥','GH-20260615-001','SL-20260615-001','海外已到仓','2','海运','6','6','0.000006'],
    ['ZPCD-20260612-001','260612-1','FBK-20260612001','喀麦隆','GH-20260612-001','SL-20260612-001','海外已到仓','1','海运','3','3','0.000003']
],[
    {label:'配舱单号',type:'text'},
...
(function(){
    ...
    if(!c)return;
    /* 只剩一列状态了，插页直接按配舱状态匹配即可 */
    c.statusBadgeCols=['配舱状态'];
...
TC['cs-issue-type']={t:'问题件类型',forceLocalHeader:true,readonlyList:true,h:['问题件类型编号','问题件类型名称','问题说明','区域类型','备注','启用状态','创建时间','创建员工编号','修改时间','修改员工编号','操作'],s:['启用','禁用'],d:[
...
/* OMS 轨迹查询改为直接复用 TMS 那张自绘页（generateTrackQueryPage 不区分调用方），
 * 只在 OMS 端隐藏时间轴上的「创建人」（内部员工信息）。写法与上面的 cs-track-query 一致。 */
TC['oms-track-query']={t:'轨迹查询',pageMode:'trackQuery',h:[],s:[],d:[]};
...
const FCL_QUERY_STATUS=['草稿','待审核','已审核','已订舱','已放仓','已装柜','已开船','已完成','异常'];
...
addPrototypeTable('fcl-cost-price','成本价','报价编号|航司|开始日期|结束日期|使用客户|状态|操作',['待生效','已生效','已失效'],[
    ['FCP-20260613001','MAERSK','2026-06-13','2026-06-30','深圳市华运达国际货运','已生效'],
    ['FCP-20260612002','COSCO','2026-06-12','2026-06-25','广州远洋进出口贸易','已生效']
],FCL_PRICE_QUERY);
...
addPrototypeTable('fcl-booking','订舱作业','订舱单号|销售指示号|船司|航线|柜型柜量|ETD|截补料时间|订舱方式|订舱员|订舱状态|操作',['待订舱','已订舱','订舱失败','取消订舱'],[
    ['FBK-20260613001','FSI-20260613001','MAERSK','西非线','40HQ×1','2026-06-20','2026-06-17 12:00','EDI','刘订舱','待订舱'],
    ['FBK-20260612002','FSI-20260612002','COSCO','西非线','20GP×2','2026-06-22','2026-06-18 18:00','官网','赵订舱','已订舱']
]);
addPrototypeTable('fcl-rule','关键业务规则','规则编号|规则名称|规则类型|适用环节|规则内容|优先级|启用状态|创建人|创建时间|操作',['启用','禁用'],[
    ['FCR-001','预定仓不产生费用','订舱规则','订舱作业','预定仓不生成应付费用且无需财务审核','1','启用','admin','2026-06-09'],
...
addPrototypeTable('fcl-edi-api','EDI/API对接','接口编号|接口名称|对接对象|业务环节|接口类型|最近同步时间|失败次数|启用状态|操作',['启用','停用','异常'],[
    ['EDI-001','船公司订舱推送','MAERSK','订舱作业','EDI','2026-06-13 12:00','0','启用'],
...
['wh-transfer-in','wh-transfer-out','wh-transfer-fee','wh-pack-rule','wh-cargo-search','wh-out-scan','wh-replenish-drop','fcl-provider-api','fcl-trial-calc','fcl-trial-calc-biz','fcl-booking-order'].forEach(function(id){
...
/* ---------- 国内库存盘点（参照海外仓库存，目的仓库→收货仓库） ---------- */
addPrototypeTable('wh-stock-check','国内库存盘点',
    '运单号|客户|收货仓库|品名|货物类型|货区货位|到库件数|已出件数|在库件数|盘点件数|入库时间|库存状态|操作',
    ['未出库','部分出库','待出库'],
    [
        ['WB-20260710001','深圳市华运达国际货运','深圳盐田仓','服装配件','普货','A区-01','20','0','20','20','2026-07-10 14:20','未出库'],
        ['WB-20260710002','广州远洋进出口贸易','广州南沙仓','手机配件','敏感货','B区-05','15','6','9','9','2026-07-10 15:10','部分出库'],
        ['WB-20260709003','东莞市鑫海物流','上海浦东仓','五金工具','普货','C区-02','8','0','8','7','2026-07-09 10:30','待出库'],
        ['WB-20260709004','上海锦程国际贸易','义乌仓','家居用品','普货','A区-07','12','9','3','3','2026-07-09 09:00','部分出库'],
        ['WB-20260708005','佛山恒通货运代理','深圳盐田仓','食品','普货','B区-12','10','0','10','10','2026-07-08 16:40','待出库']
    ],
    [
        {label:'运单号',type:'text'},
        {label:'客户',type:'text'},
        {label:'收货仓库',type:'select',options:['深圳盐田仓','广州南沙仓','上海浦东仓','义乌仓']},
        {label:'库存状态',type:'select',options:['未出库','部分出库','待出库']}
    ]
);
