...
addPrototypeTable('ow-pickup','提货预约管理',
    ...
    ['已放货','待放货'],
    [
        ['DR-20260715-023','HT-NG-20260715-00023','B2607-01','东莞市鑫海物流','拉各斯海外仓','上门提货','—','8','125.0','1860.00','已付款','2026-07-16 09:00','已放货'],
        ['DR-20260715-024','HT-SN-20260715-00088','B2607-02','上海锦程国际贸易','达喀尔海外仓','派送','120.00','12','240.0','2520.00','已付款','2026-07-16 10:00','待放货'],
        ['DR-20260714-019','HT-CI-20260714-00051','B2607-01','广州远航贸易','阿比让海外仓','派送','86.00','5','72.0','980.00','未付款','—','待放货'],
        ['DR-20260713-011','HT-NG-20260713-00040','B2606-09','深圳市华运达国际货运','拉各斯海外仓','上门提货','—','20','410.0','4260.00','已付款','2026-07-14 11:00','已放货'],
        ['DR-20260713-009','HT-CM-20260713-00033','B2606-08','东莞市鑫海物流','杜阿拉海外仓','派送','150.00','6','96.0','1180.00','部分付款','—','待放货'],
        ['DR-20260712-005','HT-NG-20260712-00021','B2606-07','上海锦程国际贸易','拉各斯海外仓','上门提货','—','15','300.0','3200.00','已付款','2026-07-13 09:00','已放货']
    ],
    [
...
addPrototypeTable('ow-arrival','海外仓到货',
    ...
    [
        ...
        ['YPCD-20260625-001','TD-20260625-001','达喀尔海外仓','海运','COSU7654321','5','26','18','18/26','部分到货'],
        ['YPCD-20260624-001','TD-20260624-001','阿比让海外仓','海运','ONEU2233445','6','30','0','0/30','部分到货'],
        ['YPCD-20260620-003','TD-20260620-003','杜阿拉海外仓','空运','—','3','12','11','11/12','部分到货']
    ],
    [
...
addPrototypeTable('ow-outbound','海外仓出库',
    ...
    ['待出库','已出库'],
    [
        ['DO-NG-20260715-00023','DR-20260715-023','HT-NG-20260715-00023','B2607-01','东莞市鑫海物流','拉各斯海外仓','上门提货','8','8','8/8','已出库','李仓管','2026-07-16 09:20','拉各斯海外仓'],
        ['DO-NG-20260713-00040','DR-20260713-011','HT-NG-20260713-00040','B2606-09','深圳市华运达国际货运','拉各斯海外仓','上门提货','20','12','12/20','待出库','王海波','2026-07-16 10:35','拉各斯海外仓'],
...
TC['ow-outbound'].listHiddenHeaders=['提单号','配舱单号'];

/* ---------- 海外仓库存(在库 / 部分提货Remaining / 异常隔离) ---------- */
addPrototypeTable('ow-inventory','海外仓库存',
    '运单号|客户|目的仓库|品名|货物类型|货区货位|到库件数|已出件数|在库件数|盘点件数|入库时间|库存状态|操作',
    ['在库','部分提货','已提清','异常隔离'],
    [
        ['WB-20260701001','东莞市鑫海物流','拉各斯海外仓','服装配件','普货','A区-A03','8','8','0','0','2026-07-05 14:20','已提清'],
        ['WB-20260701002','上海锦程国际贸易','拉各斯海外仓','手机配件','敏感货','B区-B12','15','9','6','6','2026-07-05 15:10','部分提货'],
        ['WB-20260701003','广州远航贸易','阿比让海外仓','五金工具','普货','C区-C09','5','0','5','5','2026-07-04 10:30','在库'],
        ['WB-20260630008','深圳市华运达国际货运','达喀尔海外仓','家居用品','普货','A区-A07','12','0','12','11','2026-07-03 09:00','异常隔离']
    ],
    [
        {label:'运单号',type:'text'},
        {label:'客户',type:'text'},
...
var _owInvCountRecords={
    'WB-20260701002':[
        {qty:'6',by:'David',time:'2026-07-10 09:20',note:'月度盘点，账实相符',file:'盘点单-0710.pdf'},
        {qty:'6',by:'Maria',time:'2026-07-15 16:40',note:'抽盘复核，无差异',file:'盘点照片-0715.jpg'}
    ],
    'WB-20260630008':[
...
function openOverseasInventoryDetail(id,rowIdx){
    ...
    var wb=owCell(row,headers,'运单号');
    var loc=owCell(row,headers,'货区货位');
    var arrived=parseInt(owCell(row,headers,'到库件数')||'0',10);
    var shipped=parseInt(owCell(row,headers,'已出件数')||'0',10);
    var ops=['David','Maria','Okafor','Amadou'];
    var p2=function(n){return (n<10?'0':'')+n;};
    var arrRows='';
    for(var i=0;i<arrived;i++){
        arrRows+='<tr class="border-t border-surface-100"><td class="px-3 py-2 font-medium text-primary-700 whitespace-nowrap">'+esc(wb+'-'+p2(i+1))+'</td><td class="px-3 py-2 text-text-secondary whitespace-nowrap">'+esc(loc)+'</td><td class="px-3 py-2 text-green-600 whitespace-nowrap">'+tr('已到库')+'</td><td class="px-3 py-2 text-text-secondary whitespace-nowrap">'+esc(ops[i%ops.length])+'</td><td class="px-3 py-2 text-text-secondary whitespace-nowrap">2026-07-05 '+p2(9+i%8)+':'+p2((i*7)%60)+'</td></tr>';
    }
    var outRows='';
    for(var j=0;j<shipped;j++){
...
function owSwitchInvTab(tab){
    ...
    var ba=document.getElementById('ow-inv-tabbtn-arrival'),bo=document.getElementById('ow-inv-tabbtn-outbound');
    ...
    if(tab==='outbound'){if(a)a.classList.add('hidden');if(o)o.classList.remove('hidden');if(ba)ba.className=off;if(bo)bo.className=on;}
    else{if(o)o.classList.add('hidden');if(a)a.classList.remove('hidden');if(bo)bo.className=off;if(ba)ba.className=on;}
}
/* ===== 需求4：手动盘点（盘点件数/盘点说明/上传附件） ===== */
function openOverseasInventoryManualCount(id){
    var idx=(typeof getSelectedRowIndex==='function')?getSelectedRowIndex():-1;
    if(idx<0){showToast(tr('请先选择一条库存记录'));return;}
    ...
    h+='<div class="flex flex-col gap-1.5"><label class="text-sm font-medium text-text-secondary">'+tr('盘点件数')+'<span class="text-red-500 ml-1">*</span></label><input id="ow-inv-count-qty" type="number" min="0" value="'+esc(inStock)+'" class="w-full h-9 px-3 text-sm border border-surface-200 rounded-lg bg-white"></div>';
    h+='<div class="flex flex-col gap-1.5"><label class="text-sm font-medium text-text-secondary">'+tr('盘点附件')+'</label><input id="ow-inv-count-file" type="file" class="w-full text-sm text-text-secondary file:mr-3 file:px-3 file:py-1.5 file:rounded-lg file:border-0 file:bg-primary-50 file:text-primary-700 file:text-sm file:cursor-pointer"></div>';
    h+='<div class="flex flex-col gap-1.5 md:col-span-2"><label class="text-sm font-medium text-text-secondary">'+tr('盘点说明')+'</label><textarea id="ow-inv-count-note" rows="3" class="w-full px-3 py-2 text-sm border border-surface-200 rounded-lg bg-white resize-y" placeholder="'+esc(tr('请填写盘点说明，例如：月度盘点账实相符 / 发现差异并说明原因'))+'"></textarea></div>';
    h+='</div></section>';
    h+='</div>';
    var footer='<button onclick="submitOverseasInventoryManualCount(\''+id+'\','+idx+')" class="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 cursor-pointer">'+tr('提交盘点')+'</button>'+
    ...
    owOpenModal(tr('手动盘点')+' - '+wb,'62%',h,footer);
}
function submitOverseasInventoryManualCount(id,idx){
    var qtyEl=document.getElementById('ow-inv-count-qty');
    var noteEl=document.getElementById('ow-inv-count-note');
    var fileEl=document.getElementById('ow-inv-count-file');
    var qty=qtyEl?String(qtyEl.value).trim():'';
    if(qty===''){showToast(tr('请填写盘点件数'));if(qtyEl)qtyEl.focus();return;}
    var note=noteEl?noteEl.value.trim():'';
    var fileName=(fileEl&&fileEl.files&&fileEl.files.length)?fileEl.files[0].name:'—';
    var c=TC[id]||{};
    var d=owRowData(id,idx);var row=d.row,headers=(c.h||[]);
    if(!row)return;
    var wb=owCell(row,headers,'运单号');
    var srcRow=(c.d||[]).find(function(r){return r[0]===wb;});
    if(srcRow){var iC=headers.indexOf('盘点件数');if(iC>=0)srcRow[iC]=String(qty);}
    if(!_owInvCountRecords[wb])_owInvCountRecords[wb]=[];
    _owInvCountRecords[wb].unshift({qty:String(qty),by:owCurrentOperator(),time:owNowStr(),note:note||'—',file:fileName});
    closeCrudModal();
    ...
    showToast(tr('盘点已提交')+'：'+wb+'（'+tr('盘点件数')+' '+qty+'）');
}
/* ===== 需求5：盘点记录（历史盘点列表） ===== */
function openOverseasInventoryCountRecords(id,rowIdx){
    var d=owRowData(id,rowIdx);var row=d.row,headers=(d.c.h||[]);
    if(!row){showToast(tr('未找到库存数据'));return;}
    var wb=owCell(row,headers,'运单号');
...
function owReleasePreGates(rowData,headers){
    ...
    var released=owCell(rowData,headers,'状态')==='已放货';
    return owGateGrid([
        ['1 付款到账','财务核对银行流水→可审批',paid],
        ['2 放货审批','审批通过→可生成DO',paid||released],
        ['3 生成放货单DO/已放货','有DO→仓库可开单出库',released]
    ],3);
}
...
/* 提货预约是否已放行：状态为“已放货”即视为已放行 */
function owPickupReleased(status){return status==='已放货';}
...
function openOverseasPickupDetail(id,rowIdx){
    ...
    /* footer */
    var actionBtn='';
    var payStatus=owCell(row,headers,'付款状态');
    if(status==='待放货'&&payStatus!=='已付款')actionBtn='<button onclick="showToast(\''+esc(tr('已通知客户付款'))+'\');closeCrudModal()" class="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 cursor-pointer">'+tr('通知付款')+'</button>';
    else if(status==='待放货')actionBtn='<button onclick="showToast(\''+esc(tr('放货审批已提交（需财务核对银行到账）'))+'\');closeCrudModal()" class="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 cursor-pointer">'+tr('放货审批')+'</button>';
    else actionBtn='<button onclick="showToast(\''+esc(tr('放货单DO已下发仓库'))+'\');closeCrudModal()" class="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 cursor-pointer">'+tr('下发放货单')+'</button>';
...
var _owCreateSubSel={};
/* 调整明细弹窗：已加入提货明细的运单 idx 列表 */
var _owAdjustPicked=[];
/* 调整明细弹窗：不在运单库中的历史明细行（只读展示，可移出） */
var _owAdjustExtraRows=[];
...
function openOverseasPickupAdjust(id,rowIdx){
    ...
    var savedWb={};
    saved.forEach(function(r){savedWb[r[0]]=parseInt(r[3],10)||0;});   /* 运单号 → 提货件数 */
    _owCreateSubSel={};
    _owAdjustPicked=[];   /* 提货明细：已选运单 idx 列表 */
    _owPickupWaybills.forEach(function(w,i){
        if(savedWb[w.wb]===undefined)return;
        var n=savedWb[w.wb]||w.subs.length;
        if(n>w.subs.length)n=w.subs.length;
        if(n<1)n=1;
    ...
    var bls=[],allocs=[];
    _owPickupWaybills.forEach(function(w){if(w.bl&&bls.indexOf(w.bl)<0)bls.push(w.bl);if(w.alloc&&allocs.indexOf(w.alloc)<0)allocs.push(w.alloc);});
    h+='<div class="flex flex-col gap-1.5"><label class="text-sm font-medium text-text-secondary">'+tr('提单号')+'</label><select id="ow-adjust-bl" class="w-full h-9 px-3 text-sm border border-surface-200 rounded-lg bg-white"><option value="">'+tr('全部')+'</option>'+bls.map(function(o){return '<option value="'+esc(o)+'">'+esc(o)+'</option>';}).join('')+'</select></div>';
    h+='<div class="flex flex-col gap-1.5"><label class="text-sm font-medium text-text-secondary">'+tr('批次号')+'</label><select id="ow-adjust-alloc" class="w-full h-9 px-3 text-sm border border-surface-200 rounded-lg bg-white"><option value="">'+tr('全部')+'</option>'+allocs.map(function(o){return '<option value="'+esc(o)+'">'+esc(o)+'</option>';}).join('')+'</select></div>';
    h+='</div>';
    h+='<div class="mt-3"><button type="button" onclick="owAdjustLoadWaybills()" class="h-9 px-4 rounded-lg bg-primary-600 text-white text-sm font-medium">'+tr('加载运单明细')+'</button></div>';
    h+='</section>';
    /* ② 运单明细（可加载并添加到提货明细） */
    h+='<section id="ow-adjust-src-section" class="hidden">'+owSectionTitle('② 运单明细（勾选后点“添加到提货明细”）');
    h+='<div id="ow-adjust-src-wrap"></div>';
    h+='<div class="mt-3"><button type="button" onclick="owAdjustAddPicked()" class="h-9 px-4 rounded-lg bg-primary-600 text-white text-sm font-medium">'+tr('添加到提货明细')+'</button></div>';
    h+='</section>';
    /* ③ 提货明细（可移出） */
    h+='<section>'+owSectionTitle('③ 提货明细（可点“移出”退回运单明细列表）');
    h+='<div id="ow-adjust-picked-wrap"></div>';
    h+='</section>';
    h+='</div>';
    var footer='<button onclick="submitOverseasPickupAdjust(\''+id+'\','+rowIdx+')" class="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 cursor-pointer">'+tr('保存')+'</button>'+
        '<button onclick="closeCrudModal()" class="px-4 py-2 text-sm font-medium text-text-secondary border border-surface-200 rounded-lg hover:bg-surface-50 cursor-pointer ml-2">'+tr('取消')+'</button>';
    owOpenModal(tr('调整明细')+' - '+apptNo,'88%',h,footer);
    owAdjustRenderPicked();
}
/* 运单明细（可添加）：按提单号/批次号过滤，排除已在提货明细中的 */
function owAdjustRenderSrc(){
    var blEl=document.getElementById('ow-adjust-bl');
    var allocEl=document.getElementById('ow-adjust-alloc');
    var bl=blEl?blEl.value:'';
    var alloc=allocEl?allocEl.value:'';
    var wrap=document.getElementById('ow-adjust-src-wrap');
    if(!wrap)return;
    var list=[];
    _owPickupWaybills.forEach(function(w,i){
        if(_owAdjustPicked.indexOf(i)>=0)return;
        if(bl&&w.bl!==bl)return;
        if(alloc&&w.alloc!==alloc)return;
        list.push(i);
    });
    var h='<div class="border border-surface-200 rounded-lg overflow-auto"><table class="w-full text-sm">';
    h+='<thead class="bg-surface-50 text-text-secondary"><tr>'+
        '<th class="px-3 py-2 w-10 text-center"><input type="checkbox" onclick="owAdjustToggleAllSrc(this)"></th>'+
        ['运单号','货物类型','提单号','批次号','可提货件数','体积(CBM)','重量(KG)'].map(function(x){return '<th class="px-3 py-2 text-left font-medium whitespace-nowrap">'+tr(x)+'</th>';}).join('')+
        '</tr></thead><tbody>';
    if(list.length===0){
        h+='<tr><td colspan="8" class="px-3 py-6 text-center text-text-muted">'+tr('无可添加的运单（可能已全部加入提货明细）')+'</td></tr>';
    }else{
        list.forEach(function(i){
            var w=_owPickupWaybills[i];
            h+='<tr class="border-t border-surface-100">'+
                '<td class="px-3 py-2 text-center"><input type="checkbox" class="ow-adjust-src-chk" data-idx="'+i+'"></td>'+
                ...
                '<td class="px-3 py-2 text-text-secondary whitespace-nowrap">'+esc(w.bl||'—')+'</td>'+
                '<td class="px-3 py-2 text-text-secondary whitespace-nowrap">'+esc(w.alloc||'—')+'</td>'+
...
function owAdjustLoadWaybills(){
    var sec=document.getElementById('ow-adjust-src-section');
    if(sec)sec.classList.remove('hidden');
    owAdjustRenderSrc();
    showToast(tr('已按条件加载运单明细'));
}
function owAdjustToggleAllSrc(box){
    document.querySelectorAll('.ow-adjust-src-chk').forEach(function(c){c.checked=box.checked;});
}
function owAdjustAddPicked(){
    var chks=Array.prototype.slice.call(document.querySelectorAll('.ow-adjust-src-chk')).filter(function(c){return c.checked;});
    if(chks.length===0){showToast(tr('请先勾选要添加的运单'));return;}
    chks.forEach(function(c){
        var i=parseInt(c.getAttribute('data-idx'),10);
        if(_owAdjustPicked.indexOf(i)<0){
            _owAdjustPicked.push(i);
            _owCreateSubSel[i]=owAllSubIdx(i);   /* 添加时默认全选子单 */
        }
    });
    owAdjustRenderSrc();
    owAdjustRenderPicked();
    showToast(tr('已添加到提货明细')+'（'+chks.length+'）');
}
function owAdjustRemoveExtra(k){
    _owAdjustExtraRows.splice(k,1);
    owAdjustRenderPicked();
    showToast(tr('已移出'));
}
function owAdjustRemove(i){
    var pos=_owAdjustPicked.indexOf(i);
    if(pos>=0)_owAdjustPicked.splice(pos,1);
    delete _owCreateSubSel[i];
    owAdjustRenderPicked();
    owAdjustRenderSrc();   /* 移出后回到运单明细列表 */
    showToast(tr('已移出，退回运单明细列表'));
}
/* 提货明细（可移出/子单选择） */
function owAdjustRenderPicked(){
    var wrap=document.getElementById('ow-adjust-picked-wrap');
    if(!wrap)return;
    var h='<div class="border border-surface-200 rounded-lg overflow-auto"><table class="w-full text-sm">';
    h+='<thead class="bg-surface-50 text-text-secondary"><tr>'+
        ['运单号','货物类型','到货件数','已选件数','体积(CBM)','重量(KG)','子单选择','操作'].map(function(x){return '<th class="px-3 py-2 text-left font-medium whitespace-nowrap">'+tr(x)+'</th>';}).join('')+
        '</tr></thead><tbody>';
    if(_owAdjustPicked.length===0&&_owAdjustExtraRows.length===0){
        h+='<tr><td colspan="8" class="px-3 py-6 text-center text-text-muted">'+tr('暂无提货明细，请从上方运单明细添加')+'</td></tr>';
    }else{
        /* 历史明细行（不在运单库中）：只读展示，可移出 */
        _owAdjustExtraRows.forEach(function(r,k){
            h+='<tr class="border-t border-surface-100">'+
                '<td class="px-3 py-2 font-medium text-primary-700 whitespace-nowrap">'+esc(r[0])+'</td>'+
                '<td class="px-3 py-2 text-text-secondary">'+tr(r[1]||'普货')+'</td>'+
                '<td class="px-3 py-2 font-medium">'+esc(r[2])+'</td>'+
                '<td class="px-3 py-2"><span class="font-semibold text-primary-700">'+esc(r[3])+'</span></td>'+
                '<td class="px-3 py-2">'+esc(r[4])+'</td>'+
                '<td class="px-3 py-2">'+esc(r[5])+'</td>'+
                '<td class="px-3 py-2 text-text-muted">—</td>'+
                '<td class="px-3 py-2"><a class="text-red-500 hover:text-red-600 cursor-pointer" onclick="owAdjustRemoveExtra('+k+')">'+tr('移出')+'</a></td>'+
            '</tr>';
        });
        _owAdjustPicked.forEach(function(i){
            var w=_owPickupWaybills[i];if(!w)return;
            var sel=_owCreateSubSel[i]||[];
            h+='<tr class="border-t border-surface-100">'+
                '<td class="px-3 py-2 font-medium text-primary-700 whitespace-nowrap">'+esc(w.wb)+'</td>'+
                '<td class="px-3 py-2 text-text-secondary">'+tr(w.cargo||'普货')+'</td>'+
                '<td class="px-3 py-2 font-medium">'+w.subs.length+'</td>'+
                ...
                '<td class="px-3 py-2"><a class="text-red-500 hover:text-red-600 cursor-pointer" onclick="owAdjustRemove('+i+')">'+tr('移出')+'</a></td>'+
            '</tr>';
        });
    }
    h+='</tbody></table></div>';
...
function submitOverseasPickupAdjust(id,rowIdx){
    ...
    if(!row)return;
    if(_owAdjustPicked.length===0&&_owAdjustExtraRows.length===0){showToast(tr('请至少添加一个运单到提货明细'));return;}
    var totalPcs=0,totalWeight=0,detail=[];
    /* 历史明细行按原值计入 */
    _owAdjustExtraRows.forEach(function(r){
        totalPcs+=parseInt(r[3],10)||0;
        totalWeight+=parseFloat(r[5])||0;
        detail.push(r.slice());
    });
    _owAdjustPicked.forEach(function(i){
        var w=_owPickupWaybills[i];if(!w)return;
...
function confirmOwSubSelect(i){
    ...
    /* 调整明细弹窗：子单全不选视为移出，退回运单明细列表 */
    if(!wbChk&&document.getElementById('ow-adjust-picked-wrap')){
        if(sel.length===0){closeOwSubSelectModal();owAdjustRemove(i);return;}
        owAdjustRenderPicked();
    }
    owUpdateSelPcs(i);
...
function submitOverseasPickupCreate(){
    ...
    var newRow=[apptNo,job,batch,custEl.value,(whEl&&whEl.value)||'拉各斯海外仓',pickup,fee,String(totalPcs),totalWeight.toFixed(1),due,'未付款',slot,'待放货'];
...
function openOverseasPickupEdit(id,rowIdx){
    ...
    if(released){
        ...
        h+='<div class="rounded-lg border border-primary-100 bg-primary-50 px-3 py-2 text-[11px] text-primary-700">'+tr('可修改提货方式、派送费、预约时段、派送地址与联系人信息；客户/申请号/目的仓库/状态不可修改。提货明细请用列表“调整明细”功能。')+'</div>';
    }
    /* ① 提货基本信息（只读） */
    h+='<section>'+owSectionTitle('① 提货基本信息（不可修改）')+owInfoGrid([
        ['客户',owCell(row,headers,'客户')],['提货申请号',apptNo],['目的仓库',owCell(row,headers,'目的仓库')],['当前状态',status]
    ])+'</section>';
    /* ② 提货单信息：放行前可编辑，放行后只读 */
    if(released){
    ...
    var footer=released
        ...
        '<button onclick="closeCrudModal()" class="px-4 py-2 text-sm font-medium text-text-secondary border border-surface-200 rounded-lg hover:bg-surface-50 cursor-pointer ml-2">'+tr('取消')+'</button>';
    owOpenModal(tr('修改提货信息')+' - '+apptNo,'72%',h,footer);
}
