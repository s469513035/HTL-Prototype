...
function actionConfig(action,id,rowData){
    ...
    if(action==='windowTest')return {title:'测试提醒 - '+name,fields:[{label:'提醒节点',type:'select',options:['窗口开始前60分钟','窗口截止前30分钟']},{label:'接收人',type:'select',options:['刘订仓','赵订仓','订仓主管']},{label:'通知方式',type:'select',options:['站内消息','邮件','企业微信']},{label:'测试说明',type:'textarea',span:'md:col-span-2',value:'仅发送一条测试消息，不影响正式提醒排程。'}],confirm:'发送测试'};
    if(action==='tplImport')return {title:'批量导入放仓模板 - '+name,fields:[{label:'导入文件',type:'select',options:['放仓规则表.xlsx','放仓规则表-危险品.xlsx']},{label:'冲突处理',type:'select',options:['覆盖同船公司+目的港','跳过已存在','全部新增']},{label:'导入说明',type:'textarea',span:'md:col-span-2',value:'商务/订仓主管以 Excel 维护规则表后批量导入（SOP 8.3）。'}],confirm:'开始导入'};
    /* ④ 操作执行 */
    if(action==='copyBooking')return {title:'复制相似订仓 - '+name,fields:[{label:'复制来源',type:'select',required:true,options:['同客户同船司同港口','同船司同港口','同客户最近一票']},{label:'需修改字段',type:'checkboxGroup',span:'md:col-span-2',options:['ETD','柜型柜量','船名航次','约号'],checkedOptions:['ETD','船名航次']},{label:'复制说明',type:'textarea',span:'md:col-span-2',value:'SOP 7.3 步骤4：复制相似订单可减少约 80% 重复录入。'}],confirm:'确认复制'};
    ...
    if(action==='bookingReceipt')return {title:'登记订仓回执 - '+name,fields:[{label:'订仓回执号',required:true},{label:'约号'},{label:'船名航次'},{label:'回执时间',type:'datetime-local',required:true},{label:'回执附件',type:'select',options:['待上传','booking_confirm.pdf']},{label:'回执说明',type:'textarea',span:'md:col-span-2',value:'登记后订仓状态转「已订仓」，主状态同步推进。'}],confirm:'确认登记'};
