...
addPrototypeTable('cs-ticket','工单管理',
    ...
    ['TK20260902002','5/22 发出的柜子到拉各斯了吗','催件','WB-20260522002','广州远洋进出口贸易','已到货','2','已完结','2026-09-03 16:40','许倩','2026-09-02 14:30'],
    ['TK20260905003','运费与报价单不一致','费用争议','WB-20260522003','佛山恒通货运代理','已确认','4','待处理','2026-09-06 10:05','谢舒婷','2026-09-05 11:00'],
    ['TK20260908004','外箱破损索赔','货物破损索赔','WB-20260522004','东莞市鑫海物流','已退件','2','处理中','2026-09-10 09:30','许倩','2026-09-08 16:20'],
    ['TK20260910005','清关需要提供什么资料','清关咨询','WB-20260522005','上海锦程国际贸易','已配舱','3','已完结','2026-09-11 15:10','谢舒婷','2026-09-10 08:45'],
    ...
    {label:'工单状态',type:'select',options:['待处理','处理中','已完结','已关闭']}
]);
...
function submitCsTicketReply(){
    ...
    if(row){
        ...
        /* 待处理的单有人回复了就是处理中；已完结/已关闭不动 —— 终态只有处理动作能改 */
...
function submitCsTicketProcess(){
    ...
    var row=csTicketRowByNo(no);
    if(!row){showToast(tr('未找到工单'));return;}
    /* 动作 -> 状态：完结/关闭都是终态，动作名与状态词一致 */
    var to=act==='完结'?'已完结':'已关闭';
...
function openCsTicketToIssue(id,rowIdx){
    ...
    if(st==='已完结'||st==='已关闭'){showToast(tr('终态工单不能再操作'));return;}
