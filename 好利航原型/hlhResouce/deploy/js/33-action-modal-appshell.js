...
function actionConfig(action,id,rowData){
    ...
    if(action==='entrustCancel')return {title:'取消委托 - '+name,fields:[
        ...
        {label:'取消原因',type:'select',required:true,options:['客户取消出运','客户改期','改走其他货代','爆舱无法订舱','重复录入','其他']},
        {label:'是否已订舱',type:'select',options:['否','是'],value:'否'},
        {label:'退仓成本',type:'number',value:'0'},
        {label:'取消说明',type:'textarea',span:'md:col-span-2',value:'取消后委托订单状态置为「已取消」。若已订舱，需同步到订舱部办理退仓，退仓产生的成本请录入客户历史单或新建费用单（SOP 第二十一章）。'}
    ],confirm:'确认取消委托'};
    /* ③ 订舱与仓位 */
    ...
    if(action==='slotStat')return {title:'仓位统计 - '+name,fields:[{label:'统计维度',type:'select',required:true,options:['按船东','按业务员','按航线']},{label:'统计周期',type:'select',options:['本周','本月','自定义']},{label:'统计指标',type:'checkboxGroup',span:'md:col-span-2',options:['预定仓量','实单占用','已放仓','未放仓'],checkedOptions:['预定仓量','实单占用','已放仓','未放仓']},{label:'统计说明',type:'textarea',span:'md:col-span-2',value:'对应 SOP 7.5「商务可按船东、业务、航线统计预定仓/实单/已放仓/未放仓数据」。'}],confirm:'生成统计'};
    if(action==='windowTest')return {title:'测试提醒 - '+name,fields:[{label:'提醒节点',type:'select',options:['窗口开始前60分钟','窗口截止前30分钟']},{label:'接收人',type:'select',options:['刘订舱','赵订舱','订舱主管']},{label:'通知方式',type:'select',options:['站内消息','邮件','企业微信']},{label:'测试说明',type:'textarea',span:'md:col-span-2',value:'仅发送一条测试消息，不影响正式提醒排程。'}],confirm:'发送测试'};
    if(action==='tplImport')return {title:'批量导入放仓模板 - '+name,fields:[{label:'导入文件',type:'select',options:['放仓规则表.xlsx','放仓规则表-危险品.xlsx']},{label:'冲突处理',type:'select',options:['覆盖同船公司+目的港','跳过已存在','全部新增']},{label:'导入说明',type:'textarea',span:'md:col-span-2',value:'商务/订舱主管以 Excel 维护规则表后批量导入（SOP 8.3）。'}],confirm:'开始导入'};
    /* ④ 操作执行 */
    if(action==='copyBooking')return {title:'复制相似订舱 - '+name,fields:[{label:'复制来源',type:'select',required:true,options:['同客户同船司同港口','同船司同港口','同客户最近一票']},{label:'需修改字段',type:'checkboxGroup',span:'md:col-span-2',options:['ETD','柜型柜量','船名航次','约号'],checkedOptions:['ETD','船名航次']},{label:'复制说明',type:'textarea',span:'md:col-span-2',value:'SOP 7.3 步骤4：复制相似订单可减少约 80% 重复录入。'}],confirm:'确认复制'};
    if(action==='genAgentBooking')return {title:'生成外配托书 - '+name,fields:[{label:'接收方',type:'select',required:true,options:['外配同行','一代']},{label:'接收邮箱',required:true},{label:'敏感信息剥离',type:'checkboxGroup',span:'md:col-span-2',options:['客户名称','客户联系方式','销售价格','内部备注'],checkedOptions:['客户名称','客户联系方式','销售价格','内部备注']},{label:'托书说明',type:'textarea',span:'md:col-span-2',value:'SOP 7.3 步骤3b：外配同行托书自动剥离客户名称等敏感信息后一键发送。'}],confirm:'生成并发送'};
    if(action==='bookingReceipt')return {title:'登记订舱回执 - '+name,fields:[{label:'订舱回执号',required:true},{label:'约号'},{label:'船名航次'},{label:'回执时间',type:'datetime-local',required:true},{label:'回执附件',type:'select',options:['待上传','booking_confirm.pdf']},{label:'回执说明',type:'textarea',span:'md:col-span-2',value:'登记后订舱状态转「已订舱」，主状态同步推进。'}],confirm:'确认登记'};
    if(action==='urgeDoc')return {title:'催料 - '+name,fields:[{label:'催料方式',type:'select',required:true,options:['邮件','企业微信','电话']},{label:'接收人',required:true,value:'客户补料联系人'},{label:'本次为第几次催料',type:'number',readonly:true,value:'3'},{label:'催料内容',type:'textarea',span:'md:col-span-2',value:'贵司本票货物补料已超对内截止时间，请尽快提供 SI、装箱单及报关资料，以免影响船司结单。'}],confirm:'发送催料'};
    if(action==='draftBl')return {title:'草稿件处理 - '+name,fields:[{label:'处理动作',type:'select',required:true,options:['下载草稿件','转发客户核对','登记客户确认','登记客户改单']},{label:'草稿件来源',type:'select',options:['船司邮件','船司官网']},{label:'改单费',type:'number',value:'0'},{label:'处理说明',type:'textarea',span:'md:col-span-2',value:'客户要求改单时通知船司，可能产生改单费（SOP 11.3 步骤7b）。'}],confirm:'确认处理'};
    if(action==='doSplit')return {title:'拆单（M单拆H单）- '+name,fields:[{label:'源提单号',required:true,readonly:true,value:(rowData&&getTableValueByHeader(c,rowData,'源提单号',''))||'HLHLA260613001'},{label:'拆分方式',type:'select',required:true,options:['按柜型拆分','按柜量拆分','按费用拆分','人工拆分']},{label:'拆分数量',type:'number',required:true,value:'2'},{label:'费用分摊方式',type:'select',required:true,options:['按柜','按票','按重量','手工指定']},{label:'收货人明细',type:'textarea',span:'md:col-span-2',placeholder:'每行一个收货人，与拆分数量一致'},{label:'拆单说明',type:'textarea',span:'md:col-span-2',value:'遵循船公司拆单逻辑；拆出的 HBL 会打上拆单标记，放单时强制转人工审核（SOP 17.2）。'}],confirm:'确认拆单'};
    if(action==='doMerge')return {title:'并单（H单合M单）- '+name,fields:[{label:'待合并提单',type:'checkboxGroup',span:'md:col-span-2',required:true,options:['HLHDK260612002 / Dakar Trading','HLHDK260612003 / Dakar Trading','HLHDK260612004 / Dakar Retail']},{label:'目标提单号',value:'系统自动生成',readonly:true},{label:'关联报关方式',type:'select',options:['合并报关','单独报关']},{label:'并单说明',type:'textarea',span:'md:col-span-2',value:'多张 HBL 合并为 1 张 MBL，合并申报可节省费用（SOP 11.4）。'}],confirm:'确认并单'};
    if(action==='feeAllocate')return {title:'费用分摊 - '+name,fields:[{label:'分摊方式',type:'select',required:true,options:['按柜','按票','按重量','手工指定']},{label:'分摊费用项',type:'checkboxGroup',span:'md:col-span-2',options:['海运费','附加费','报关费','拖车费'],checkedOptions:['海运费','附加费']},{label:'分摊说明',type:'textarea',span:'md:col-span-2',value:'拆单后应付费用按所选方式分摊到各 HBL（DES-FCL BLS-03）。'}],confirm:'确认分摊'};
    /* ⑤ 财务与结算 */
    if(action==='submitAppeal')return {title:'提交申诉 - '+name,fields:[{label:'关联账单号',required:true,readonly:true,value:(rowData&&getTableValueByHeader(c,rowData,'关联账单号',''))||'FBE-20260613001'},{label:'账单付款情况',type:'select',required:true,options:['已付款','未付款','部分付款']},{label:'申诉类型',type:'select',required:true,options:['附加费多计','汇率差异','开船延误产生的额外费用','重复计费','单价与价格表不符','塞港费争议','其他']},{label:'争议金额',type:'number',required:true},{label:'申诉周期(天)',type:'number',value:'90'},{label:'到期日',type:'date'},{label:'申诉原因',type:'textarea',required:true,span:'md:col-span-2'},{label:'风险提示',type:'textarea',readonly:true,span:'md:col-span-2',value:'SOP 15.4：先付款再申诉，避免逾期影响后续业务。账单未付款时系统给出强提示，仍可强制提交但会记录该情形。'}],confirm:'提交申诉'};
    if(action==='appealResult')return {title:'登记申诉结果 - '+name,fields:[{label:'申诉结论',type:'select',required:true,options:['已通过','已驳回']},{label:'处理结果',type:'select',options:['退款','抵扣下次账单','维持原账单']},{label:'结果金额',type:'number'},{label:'关闭时间',type:'datetime-local'},{label:'船司回复',type:'textarea',span:'md:col-span-2'}],confirm:'登记结果'};
    if(action==='genOffsetFee')return {title:'生成抵扣费用 - '+name,fields:[{label:'生成方式',type:'select',required:true,options:['负数费用单（抵扣下次账单）','应收退款单']},{label:'抵扣金额',type:'number',required:true},{label:'抵扣船公司',type:'select',options:['MAERSK','COSCO','MSC','CMA CGM']},{label:'生成说明',type:'textarea',span:'md:col-span-2',value:'SOP 15.2 步骤5：申诉成功后录入负数费用，便于在下期账单中抵扣。'}],confirm:'生成'};
    if(action==='submitPr')return {title:'提交请款审批 - '+name,fields:[{label:'期望付款时间',type:'date',required:true},{label:'最晚付款期限',type:'date',readonly:true,value:'2026-06-30'},{label:'审批人',type:'select',options:['财务主管','财务总监']},{label:'锁定提示',type:'textarea',readonly:true,span:'md:col-span-2',value:'SOP 16.2：期望付款时间由操作录入，提交后不可修改；实际付款时间由财务录入，系统按两者差异做资金盘点。'}],confirm:'提交审批'};
    if(action==='mergePay')return {title:'合并付款 - '+name,fields:[{label:'合并条件',type:'select',required:true,options:['同船东+同币种'],value:'同船东+同币种',readonly:true},{label:'待合并请款单',type:'checkboxGroup',span:'md:col-span-2',required:true,options:['FPR-20260612002 / COSCO / USD 5,180','FPR-20260611003 / COSCO / USD 2,460']},{label:'合并批次号',value:'系统自动生成',readonly:true},{label:'合并说明',type:'textarea',span:'md:col-span-2',value:'SOP 16.1 步骤3：同一船东、同一币种的多笔请款合并付款，节省手续费。'}],confirm:'确认合并'};
    if(action==='genPayment')return {title:'生成付款单 - '+name,fields:[{label:'付款账户',type:'select',required:true,options:['招商银行 6225****8888','中国银行 4563****1234']},{label:'实际付款时间',type:'date',required:true},{label:'银行水单',type:'select',options:['待上传','receipt_202606_001.pdf']},{label:'生成说明',type:'textarea',span:'md:col-span-2',value:'付款完成后自动核销该笔应付，并弹消息通知操作员（SOP 16.1 步骤5）。'}],confirm:'生成付款单'};
    if(action==='releaseJudge')return {title:'放单判定 - '+name,fields:[{label:'客户黑名单',type:'select',readonly:true,value:(rowData&&getTableValueByHeader(c,rowData,'客户黑名单',''))||'否'},{label:'当票应收是否核销',type:'select',options:['是','否']},{label:'历史未核销单数',type:'number',readonly:true,value:(rowData&&getTableValueByHeader(c,rowData,'历史未核销单数',''))||'0'},{label:'拆单/分批付款标记',type:'select',options:['无','有']},{label:'判定结果',type:'select',required:true,options:['自动放单','转人工审核','强拦截']},{label:'判定依据',type:'textarea',readonly:true,span:'md:col-span-2',value:'① 黑名单→强拦截；② 当票未核销→人工；③ 有历史未核销→人工；④ 拆单部分核销/分批付款→人工并高亮；⑤ 全部通过→自动放单（≤2小时）。'}],confirm:'执行判定'};
    if(action==='calcCommission')return {title:'核算提成 - '+name,fields:[{label:'核算月份',type:'month',required:true},{label:'核算范围',type:'select',options:['已勾选票单','当前筛选结果','本月全部已完结']},{label:'提成比例',type:'number',value:'8'},{label:'核算口径',type:'textarea',readonly:true,span:'md:col-span-2',value:'毛利 = 应收 − 成本；须满足：收款到位 + 财务对账完成 + 该票无未关闭申诉（SOP 18.1）。'}],confirm:'开始核算'};
    if(action==='grantCommission')return {title:'确认发放 - '+name,fields:[{label:'发放月份',type:'month',required:true},{label:'发放方式',type:'select',options:['随工资发放','单独发放']},{label:'发放时间',type:'date',required:true},{label:'发放说明',type:'textarea',span:'md:col-span-2',value:'仅「可发放标记 = 是」的提成单可发放；存在未关闭申诉的票单会被自动排除。'}],confirm:'确认发放'};
    if(action==='perfBoard')return {title:'业绩看板 - '+name,fields:[{label:'查看维度',type:'select',required:true,options:['个人业绩卡','票单明细','部门排名','大区排名','近6月趋势']},{label:'统计周期',type:'select',options:['本月','本季','本年']},{label:'看板说明',type:'textarea',readonly:true,span:'md:col-span-2',value:'SOP 18.2：业务员实时可见应收款、公司成本、利润、提成预估，并可与自己的记账逐票核对，简化工作、加深信任。'}],confirm:'查看看板'};
