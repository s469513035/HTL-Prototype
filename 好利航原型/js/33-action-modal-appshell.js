...
function actionConfig(action,id,rowData){
    ...
    if(action==='specialPrice')return {title:'特价申请 - '+name,fields:[{label:'申请类型',type:'radioGroup',required:true,options:['单价模式','总价模式'],value:'单价模式',span:'md:col-span-2'},{label:'申请价格',required:true,placeholder:'请输入特价金额'},{label:'币别',type:'select',required:true,options:['CNY','USD','EUR']},{label:'申请原因',type:'textarea',required:true,span:'md:col-span-2'}],confirm:'提交特价申请'};
    if(action==='sendInstruction')return {title:'发送指令',fields:[{label:'指令类型',type:'select',required:true,options:['库内盘点','开箱验货','出货复核','收货复核'],span:'md:col-span-2'},{label:'指令内容',type:'textarea',required:true,span:'md:col-span-2'},{label:'备注',type:'textarea',span:'md:col-span-2'}],confirm:'确认'};
