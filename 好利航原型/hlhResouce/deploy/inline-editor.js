...
// -------- 闸门：URL 必须带 ?edit=1（或 ?edit）才启用 --------
try {
  var _urlParams = new URLSearchParams(location.search);
  if (!_urlParams.has('edit')) {
    // 静默不装载。留空 API 防止 InlineEditor.reapply() 报错
    window.InlineEditor = { enabled: false, reapply: function(){}, activate: function(){}, deactivate: function(){} };
    return;
  }
} catch(e) {}
...
// -------- 对外 reapply：主原型 innerHTML 重刷后重新应用修改 --------
function reapply(scope){
  const root = (scope && scope.querySelectorAll) ? scope : document;
  // 清 dupe 副本，防止 MutationObserver 反复触发时叠加
  root.querySelectorAll('[data-ed-clone-of]').forEach(e => e.remove());
  root.querySelectorAll('[data-dupe-applied]').forEach(e => { delete e.dataset.dupeApplied; });
  root.querySelectorAll('[data-edited-applied]').forEach(e => { delete e.dataset.editedApplied; });
  applyState();
}
