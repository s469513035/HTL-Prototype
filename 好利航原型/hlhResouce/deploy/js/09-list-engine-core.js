...
function fieldSelectOptions(id,hd,c){
    ...
    if(hd==='风控级别')return ['禁止操作','二次确认','弹出提示'];
    if(hd==='生成问题件')return ['是','否'];
...
function renderToolbarAction(action,id){
    ...
    else if(action.key==='invoiceIssue')click='openFinInvoiceIssue(\''+id+'\')';
    else if(action.key==='invoiceRedFlush')click='redFlushFinInvoice(\''+id+'\')';
...
function getToolbarActions(id){
    ...
    if(id==='wb-op-instruction'){
        return [
            {key:'search',label:'查询数据',variant:'primary'},
            {key:'opInstructionAttach',label:'查看附件',variant:'primary'}
        ];
    }
    if(id==='cs-issue-type'){
        return [
    ...
    if(id==='crm-cust'){
        /* 已去掉「导出数据」；「删除」由 _rowNoDeleteIds 阻止引擎自动追加 */
        return [
            {key:'search',label:'查询数据',variant:'primary'},
            {type:'add',label:'新增数据',variant:'primary'},
            {type:'edit',label:'编辑数据'},
    ...
    if(id==='oms-bill'){
        return [
            ...
            {key:'omsBillDownload',label:'账单下载'},
            {key:'export',label:'导出数据'}
        ];
    }
    if(id==='fin-fee-mgmt'){
    ...
    if(id.indexOf('fcl-')===0){
        ...
        if(id==='fcl-agent-bill'){
            ...
            base.push({key:'agentBillReconcile',label:'对账',variant:'primary'},
        ...
        if(id==='fcl-agent-cost'){
            /* 成本行由代理账单落下来，页面上不给新增/编辑；查看走行内「查看」 */
            for(var ci=base.length-1;ci>=0;ci--)if(base[ci].type==='view'||base[ci].type==='add'||base[ci].type==='edit')base.splice(ci,1);
    ...
    /* 发票管理：单子由各业务侧「申请开票」推过来，这里不手工新增/编辑，
     * 只做开票、红冲（开错的已开票）、作废（还没开的申请） */
    if(id==='fin-invoice'){
        return [
            {key:'search',label:'查询数据',variant:'primary'},
            {key:'invoiceIssue',label:'开票',variant:'primary'},
            {key:'invoiceRedFlush',label:'红冲'},
            {key:'invoiceVoid',label:'作废',variant:'danger'},
            {key:'export',label:'导出数据'}
        ];
    }
    if(id==='perm-user'){
